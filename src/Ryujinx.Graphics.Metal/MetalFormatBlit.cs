...
vertex BlitVertexOut blit_v(uint vid [[vertex_id]], const device BlitParams & blit [[buffer(BLIT_PARAMS_INDEX)]]) {
    ...
    out.pos = float4(pos * 2.0 - 1.0, 0.0, 1.0);
    // Metal NDC is y-up; framebuffer and texture coordinates are top-left origin.
    // Map ndc +1 (top) -> srcUvRect top so the copy preserves vertical orientation
    // exactly like copyFromTexture: (src row Y -> dst row Y).
    out.uv = blit.srcUvRect.xy + float2(0.5 + 0.5 * pos.x, 0.5 - 0.5 * pos.y) * blit.srcUvRect.zw;
    return out;
}

fragment float4 blit_f(BlitVertexOut in [[stage_in]], texture2d<float> src [[texture(BLIT_TEXTURE_INDEX)]]) {
...
";
        ...
        public void Copy(MetalTexture src, MetalTexture dst, int srcX, int srcY, int dstX, int dstY, int width, int height)
        {
            ...
            if (srcHandle == nint.Zero || dstHandle == nint.Zero || width <= 0 || height <= 0)
            {
                return;
            }
            ...
            try
            {
                ...
                unsafe
                {
                    MTLViewport viewport = new(dstX, dstY, width, height, 0.0, 1.0);
                    MetalBindings.objc_msgSend_void(encoder, MetalBindings.SelSetViewport, &viewport);
                }

                float uvX = (float)srcX / Math.Max(1, src.Width);
                float uvY = (float)srcY / Math.Max(1, src.Height);
                float uvW = (float)width / Math.Max(1, src.Width);
                float uvH = (float)height / Math.Max(1, src.Height);

                unsafe
                {
                    float[] blitParams = { uvX, uvY, uvW, uvH };
                    fixed (float* p = blitParams)
                    {
                        MetalBindings.objc_msgSend_void(encoder, MetalBindings.SelSetVertexBytesLengthAtIndex, p, (nuint)(sizeof(float) * 4), (nuint)0);
                    }
                }
                ...
                if (!_probed)
                {
                    ProbeCopyResult(commandBuffer, dst, dstX, dstY, width, height);
                }
            }
            finally
            {
