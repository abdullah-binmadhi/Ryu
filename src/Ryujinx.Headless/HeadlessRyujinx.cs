    ...
    public partial class HeadlessRyujinx
    {
        ...
        static void Load(string[] originalArgs, Options option)
        {
            ...
            if (!File.Exists(Path.Combine(AppDataManager.KeysDirPath, "prod.keys")))
            {
                ...
                {
                    Logger.Error?.Print(LogClass.Application, "Keys not found! Ensure prod.keys and title.keys are installed.");
                }
            }
            ...
            if (option.ExtractShaders || option.PrecompileShaders || option.PrebakeAll || option.Prebake)
            {
                if (string.IsNullOrEmpty(option.InputPath))
                {
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine("[Ryu] Error: Please specify a valid ROM path (e.g. Ryu --prebake-all \"/path/to/game.xci\")");
                    Console.ResetColor();
                    Environment.Exit(1);
                    return;
                }
            ...
            if (!string.IsNullOrEmpty(option.InstallFirmwarePath))
            {
                ...
                try
                {
                    ...
                    Logger.Notice.Print(LogClass.Application, $"Successfully installed Firmware version: {installedVersion?.VersionString}!");
                }
                catch (Exception ex)
                {
                    Logger.Error?.Print(LogClass.Application, $"Failed to install firmware: {ex.Message}");
                }
            ...
            if (_contentManager.GetCurrentFirmwareVersion() == null)
            {
                ...
                if (fwToInstall != null)
                {
                    ...
                    try
                    {
                        _contentManager.InstallFirmware(fwToInstall);
                        SystemVersion installedVersion = _contentManager.GetCurrentFirmwareVersion();
                        Logger.Notice.Print(LogClass.Application, $"Auto-installed Firmware: {installedVersion?.VersionString}");
                    }
                    catch (Exception ex)
                    {
                        Logger.Warning?.Print(LogClass.Application, $"Auto firmware install skipped: {ex.Message}");
                    }
                }
            }
            ...
            if (option.InputPath == null)
            {
                Logger.Error?.Print(LogClass.Application, "Please provide a Nintendo Switch ROM (XCI, NSP, NCA, or NRO) to load.");
                return;
            }
            ...
            else if (File.Exists(path))
            {
                switch (Path.GetExtension(path).ToLowerInvariant())
                {
                    ...
                    case ".pfs0":
                        ...
                        break;
                    default:
                        Logger.Info?.Print(LogClass.Application, "Loading as Homebrew / Executable.");
                        try
                        {
                        ...
                        {
                            Logger.Error?.Print(LogClass.Application, "The specified file is not supported.");
                            _emulationContext.Dispose();
                            return false;
                        }
